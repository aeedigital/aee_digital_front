...
const Regional_Card: React.FC<CardProps> = ({ nome, pais, regionalId }) => {
  ...
  useEffect(() => {
    async function fetchCentrosCount() {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/centros?REGIONAL=${regionalId}`);
      ...
      const summaryRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/regionais/${regionalId}/summaries?fields=FORM_ID,CENTRO_ID,createdAt,updatedAt`);
